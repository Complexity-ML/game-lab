...
function showPreview() {
  ...
  status.update({ stage: 'Build', message: 'Building the native GAME LAB Electron application...', percent: 52 })
  status.appendLog('5% Release', 'Checking the latest GAME LAB release...')
  ...
  status.appendLog('52% Build', 'Building the native GAME LAB Electron application...')
  setInstalling(true)
}
...
async function startTauriIntegration() {
  ...
  footer.button.addEventListener('click', async () => {
    ...
    try {
      ...
      status.update({
        stage: 'Installed',
        message: 'GAME LAB is installed and launching. Future updates are available from Settings.',
        percent: 100,
      })
