...
const REPOSITORY: &str = "Complexity-ML/game-lab";
...
fn begin_install() -> Result<InstallGuard, String> {
    INSTALL_IN_PROGRESS
        ...
        .map(|_| InstallGuard)
        .map_err(|_| {
            "GAME LAB Setup is already installing. Keep this window open to follow its progress."
                .to_string()
        })
}
...
fn latest_app_release() -> Result<GitHubRelease, String> {
    let api_result = (|| {
        github_releases()?
            ...
            .ok_or_else(|| "GAME LAB has no published stable application release yet".to_string())
    })();
    api_result.or_else(|api_error| {
        ...
        let tag_name = response
            ...
            .to_string();
        let asset_names = [
            "GAME-LAB-Setup-arm64-helper",
            "GAME-LAB-Setup-arm64-helper.sha256",
            "GAME-LAB-Setup-x64-helper.exe",
            "GAME-LAB-Setup-x64-helper.exe.sha256",
            "GAME-LAB-Setup-x64.AppImage",
            "GAME-LAB-Setup-x64.AppImage.sha256",
        ];
        Ok(GitHubRelease {
            tarball_url: format!(
...
fn latest_setup_release() -> Result<GitHubRelease, String> {
    github_releases()?
        ...
        .ok_or_else(|| "GAME LAB has no published Setup release yet".to_string())
}
...
fn install_root() -> Result<PathBuf, String> {
    #[cfg(target_os = "windows")]
    {
        return dirs::data_local_dir()
            .map(|path| path.join("GAME LAB").join("setup-data"))
            .ok_or_else(|| "No local application-data directory is available".to_string());
    }
    #[cfg(not(target_os = "windows"))]
    dirs::data_local_dir()
        .map(|path| path.join("GAME LAB Setup"))
        .ok_or_else(|| "No local application-data directory is available".to_string())
}
...
fn electron_user_data() -> Result<PathBuf, String> {
    #[cfg(target_os = "macos")]
    {
        return dirs::home_dir()
            .map(|path| path.join("Library/Application Support/GAME LAB"))
            .ok_or_else(|| "No home directory is available".to_string());
    }
    #[cfg(target_os = "windows")]
    {
        return dirs::config_dir()
            .map(|path| path.join("GAME LAB"))
...
fn app_destination() -> Result<PathBuf, String> {
    ...
    {
        return dirs::home_dir()
            .map(|path| path.join("Applications/GAME LAB.app"))
            .ok_or_else(|| "No home directory is available".to_string());
    }
    #[cfg(target_os = "windows")]
    {
        return dirs::data_local_dir()
            .map(|path| path.join("Programs/GAME LAB"))
            .ok_or_else(|| "No local application-data directory is available".to_string());
    }
    #[cfg(target_os = "linux")]
    {
        return dirs::data_local_dir()
            .map(|path| path.join("GAME LAB").join("app"))
            .ok_or_else(|| "No XDG application-data directory is available".to_string());
    }
    #[allow(unreachable_code)]
    Err("GAME LAB Setup currently supports macOS, Windows and Linux".to_string())
}
...
fn helper_destination() -> Result<PathBuf, String> {
    ...
    Ok(electron_user_data()?
        .join("installer")
        .join(format!("game-lab-setup{extension}")))
}
...
fn synchronize_electron_channel(channel: &str) -> Result<(), String> {
    ...
    fs::write(path, normalized_channel(Some(channel)))
        .map_err(|error| format!("Unable to synchronize the GAME LAB update channel: {error}"))
}

fn setup_helper_asset() -> Result<&'static str, String> {
    match (env::consts::OS, env::consts::ARCH) {
        ("macos", "aarch64") => Ok("GAME-LAB-Setup-arm64-helper"),
        ("macos", "x86_64") => Ok("GAME-LAB-Setup-x64-helper"),
        ("windows", "x86_64") => Ok("GAME-LAB-Setup-x64-helper.exe"),
        ("windows", "aarch64") => Ok("GAME-LAB-Setup-arm64-helper.exe"),
        ("linux", "x86_64") => Ok("GAME-LAB-Setup-x64.AppImage"),
        _ => Err(format!(
...
fn relaunch_latest_setup(
    ...
    emit(
        ...
        "Setup update",
        format!(
            "Updating GAME LAB Setup to {} before continuing…",
            release.tag_name
        ),
        8,
    );
...
fn built_application(source: &Path) -> PathBuf {
    ...
    source.join("release").join(folder).join("GAME LAB.app")
}
...
fn copy_application(source: &Path, destination: &Path) -> Result<(), String> {
    ...
    let status = background_command("robocopy")
        ...
        .arg("/NDL")
        .status();
    let status = status.map_err(|error| format!("Unable to copy GAME LAB: {error}"))?;
    #[cfg(any(target_os = "macos", target_os = "linux"))]
...
fn launch_application(destination: &Path) -> Result<(), String> {
    ...
    background_command("/usr/bin/open")
        ...
        .map_err(|error| error.to_string())?;
    #[cfg(target_os = "windows")]
    background_command(destination.join("GAME LAB.exe"))
        .spawn()
        .map_err(|error| error.to_string())?;
    #[cfg(target_os = "linux")]
...
fn install_linux_integration(destination: &Path, source: &Path) -> Result<(), String> {
    ...
    let icon = destination.join("resources").join("game-lab.png");
    ...
    let desktop_entry = format!(
        "[Desktop Entry]\nType=Application\nVersion=1.0\nName=GAME LAB\nComment=Private game operations and agent studio\nExec=\"{}\"\nIcon={}\nTerminal=false\nCategories=Development;Game;\nStartupWMClass=GAME LAB\n",
        executable.display().to_string().replace('"', "\\\""),
        icon.display()
    );
    fs::write(applications.join("game-lab.desktop"), desktop_entry)
...
fn install_windows_integration(destination: &Path, release_tag: &str) -> Result<(), String> {
    let executable = destination.join("GAME LAB.exe");
    ...
    let uninstall_script = install_root()?.join("uninstall-game-lab.ps1");
    let uninstall_body = format!(
        "$ErrorActionPreference = 'SilentlyContinue'\n\
         Stop-Process -Name 'GAME LAB' -Force\n\
         Start-Sleep -Milliseconds 400\n\
         Remove-Item -LiteralPath '{}' -Recurse -Force\n\
         $shell = New-Object -ComObject WScript.Shell\n\
         Remove-Item -LiteralPath ([IO.Path]::Combine([Environment]::GetFolderPath('Programs'), 'GAME LAB.lnk')) -Force\n\
         Remove-Item -LiteralPath ([IO.Path]::Combine([Environment]::GetFolderPath('Desktop'), 'GAME LAB.lnk')) -Force\n\
         Remove-Item -LiteralPath 'HKCU:\\Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\GAME LAB' -Recurse -Force\n",
        powershell_literal(destination)
    );
    fs::write(&uninstall_script, uninstall_body)
        .map_err(|error| format!("Unable to create the GAME LAB uninstaller: {error}"))?;
    ...
    let script = format!(
        "$ErrorActionPreference = 'Stop'; \
         ...
         $shell = New-Object -ComObject WScript.Shell; \
         foreach ($link in @([IO.Path]::Combine([Environment]::GetFolderPath('Programs'), 'GAME LAB.lnk'), [IO.Path]::Combine([Environment]::GetFolderPath('Desktop'), 'GAME LAB.lnk'))) {{ \
           $shortcut = $shell.CreateShortcut($link); \
           $shortcut.TargetPath = $exe; \
           $shortcut.WorkingDirectory = '{destination_literal}'; \
           $shortcut.IconLocation = \"$exe,0\"; \
           $shortcut.Description = 'GAME LAB private game operations and agent studio'; \
           $shortcut.Save(); \
         }}; \
         $key = 'HKCU:\\Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\GAME LAB'; \
         New-Item -Path $key -Force | Out-Null; \
         New-ItemProperty -Path $key -Name DisplayName -Value 'GAME LAB' -PropertyType String -Force | Out-Null; \
         ...
         New-ItemProperty -Path $key -Name URLInfoAbout -Value 'https://www.complexity-ai.fr/game-lab' -PropertyType String -Force | Out-Null; \
...
fn schedule_windows_setup_cleanup() -> Result<(), String> {
    ...
    let script = format!(
        "$ErrorActionPreference = 'SilentlyContinue'; \
         ...
         Remove-Item -LiteralPath ([IO.Path]::Combine([Environment]::GetFolderPath('Programs'), 'GAME LAB Setup.lnk')) -Force; \
         Remove-Item -LiteralPath ([IO.Path]::Combine([Environment]::GetFolderPath('Desktop'), 'GAME LAB Setup.lnk')) -Force; \
         $setupDirectories = @(); \
         Get-ItemProperty 'HKCU:\\Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\*' | \
           Where-Object {{ $_.DisplayName -eq 'GAME LAB Setup' }} | \
           ForEach-Object {{ \
             if ($_.InstallLocation -and ([IO.Path]::GetFileName($_.InstallLocation.TrimEnd('\\')) -eq 'GAME LAB Setup')) {{ $setupDirectories += $_.InstallLocation }}; \
...
fn prepare_application_handoff(app: &AppHandle) -> Result<(), String> {
    if let Some(window) = app.get_webview_window("main") {
        window
            .hide()
            .map_err(|error| format!("Unable to hide GAME LAB Setup: {error}"))?;
    }
    app.set_activation_policy(tauri::ActivationPolicy::Accessory)
        .map_err(|error| format!("Unable to remove GAME LAB Setup from the Dock: {error}"))?;
...
fn stop_running_application() -> Result<(), String> {
    ...
    let _ = background_command("taskkill")
        .args(["/IM", "GAME LAB.exe", "/T", "/F"])
        .output();
    for _ in 0..50 {
        let running = background_command("tasklist")
            .args(["/FI", "IMAGENAME eq GAME LAB.exe", "/NH"])
            .output()
            .map(|output| {
    ...
    Err("GAME LAB is still running. Close the application, then retry the update.".to_string())
}
...
fn activate_application(next: &Path, destination: &Path, previous: &Path) -> Result<(), String> {
    ...
    if had_previous {
        rename_application_directory(destination, previous)
            .map_err(|error| format!("Unable to preserve the previous GAME LAB build: {error}"))?;
    }
    if let Err(error) = rename_application_directory(next, destination) {
        ...
        return Err(format!(
            "Unable to activate the new GAME LAB build; the previous build was restored: {error}"
        ));
    }
    Ok(())
}
...
fn perform_install(app: &AppHandle, channel: &str) -> Result<InstallResult, String> {
    ...
    emit(
        ...
        "Release",
        if channel == "main" {
            "Checking the latest GAME LAB main commit…"
        } else {
            "Checking the latest GAME LAB release…"
        },
        5,
    );
    ...
    emit(
        app,
        "Build",
        "Building the native GAME LAB Electron application…",
        52,
    );
    #[cfg(target_os = "macos")]
    run_npm(
    ...
    emit(
        app,
        "Complete",
        "GAME LAB is ready. Launching the application…",
        100,
    );
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .expect("error while running GAME LAB Setup");
}
...
mod tests {
    ...
    fn activates_a_build_and_keeps_one_rollback_copy() {
        ...
        let destination = root.path().join("GAME LAB");
        let next = root.path().join("GAME LAB.next");
        let previous = root.path().join("GAME LAB.previous");
