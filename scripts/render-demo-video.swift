...
let slides = [
    Slide(image: "docs/assets/game-lab-blank-workbench.png", title: "1 · Honest blank workbench", caption: "No fake success and no checkpoint exist before a real data source is selected.", seconds: 18),
    Slide(image: "docs/assets/game-lab-datahub-connected.png", title: "2 · Trusted DataHub context", caption: "The official MCP server exposes bounded schema, classification and lineage reads from local DataHub OSS.", seconds: 28),
    Slide(image: "docs/assets/game-lab-proposal-review.png", title: "3 · Reviewable agent proposal", caption: "The agent returns a strict card-and-edge diff. The committed graph is unchanged until explicit approval.", seconds: 35),
    Slide(image: "docs/assets/game-lab-reviewed-pipeline.png", title: "4 · Atomic graph correction", caption: "The approved revision adds profile memory, impact analysis, PII protection and a Human Review gate.", seconds: 34),
    Slide(image: "docs/assets/game-lab-version-checkpoint.png", title: "5 · Evidence inherited by the next agent", caption: "The checkpoint preserves get_entities, list_schema_fields and get_lineage provenance for replay and audit.", seconds: 25),
]
...
guard writer.canAdd(input) else { throw NSError(domain: "GAME LAB demo", code: 1, userInfo: [NSLocalizedDescriptionKey: "AVAssetWriter refused the video input"]) }
writer.add(input)
guard writer.startWriting() else { throw writer.error ?? NSError(domain: "GAME LAB demo", code: 2) }
writer.startSession(atSourceTime: .zero)

let images: [NSImage] = try slides.map { slide in
    let url = root.appendingPathComponent(slide.image)
    guard let image = NSImage(contentsOf: url) else {
        throw NSError(domain: "GAME LAB demo", code: 3, userInfo: [NSLocalizedDescriptionKey: "Unable to read \(url.path)"])
    }
    return image
}
...
for (slideIndex, slide) in slides.enumerated() {
    ...
    for localFrame in 0..<frameCount {
        waitUntilReady()
        guard let pool = adaptor.pixelBufferPool else { throw NSError(domain: "GAME LAB demo", code: 4) }
        var optionalBuffer: CVPixelBuffer?
        guard CVPixelBufferPoolCreatePixelBuffer(nil, pool, &optionalBuffer) == kCVReturnSuccess, let buffer = optionalBuffer else {
            throw NSError(domain: "GAME LAB demo", code: 5)
        }
        CVPixelBufferLockBaseAddress(buffer, [])
        defer { CVPixelBufferUnlockBaseAddress(buffer, []) }
        guard let baseAddress = CVPixelBufferGetBaseAddress(buffer) else { throw NSError(domain: "GAME LAB demo", code: 6) }
        ...
        ) else { throw NSError(domain: "GAME LAB demo", code: 7) }
        ...
        guard adaptor.append(buffer, withPresentationTime: presentationTime) else {
            throw writer.error ?? NSError(domain: "GAME LAB demo", code: 8)
        }
        frameIndex += 1
    }
}
...
semaphore.wait()
guard writer.status == .completed else { throw writer.error ?? NSError(domain: "GAME LAB demo", code: 9) }
