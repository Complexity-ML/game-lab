...
GAME LAB keeps Electron's standard Windows frame. The system title bar and its native minimize, maximize and close controls remain outside the GAME LAB interface, so the application header does not need to imitate or reserve space for them.
...
Closing the last window exits GAME LAB on Windows and runs the normal workspace cleanup path. SQLite is stored under Electron's per-user `userData` directory; no hard-coded macOS path is used.
