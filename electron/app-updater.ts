...
export class AppUpdateController {
  ...
  constructor(private readonly options: {
    ...
    const unavailableMessage = options.platform === 'win32'
      ? 'Updates require a signed GAME LAB Windows installer. Unsigned development packages stay offline.'
      : options.platform === 'darwin'
        ? 'Updates are available only in a signed GAME LAB macOS application.'
  ...
  private bindEvents() {
    ...
    this.updater.on('update-available', (info: UpdateInfo) => this.transition('available', `GAME LAB ${info.version} is available. Download requires your approval.`, { availableVersion: info.version }))
    this.updater.on('update-not-available', (info: UpdateInfo) => this.transition('ready', `GAME LAB ${this.options.currentVersion} is current on the ${this.status.channel} channel.`, { availableVersion: info.version }))
    ...
    this.updater.on('update-downloaded', (event: UpdateDownloadedEvent) => this.transition('downloaded', `GAME LAB ${event.version} is downloaded. The operating system will enforce its code signature during installation.`, { availableVersion: event.version, progress: 100 }))
  ...
  async initialize() {
    ...
    if (!verified) {
      this.publish(withUpdateCapabilities({ ...this.status, phase: 'blocked', currentSignatureVerified: false, downloadedSignatureEnforced: true, message: 'Updates are blocked because this GAME LAB build is not signed with Apple Developer ID.' }))
