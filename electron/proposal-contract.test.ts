...
describe('strict provider proposal contract', () => {
  ...
  it('repairs only non-structural add-card metadata when the provider returns null', () => {
    ...
    expect(result.actions[0]).toMatchObject({ node_id: 'mask-email', kind: 'transform', label: 'Action Transform', description: 'Agent-proposed Action Transform awaiting graph review.', owner: 'GAME LAB Agent' })
  })
