...
export function parseCatalogConnectorManifest(value: unknown): CatalogConnectorManifest {
  ...
  const contract = input.contract === undefined || input.contract === 'game-lab.catalog.v1' ? 'game-lab.catalog.v1' : undefined
  ...
  if (!kind || !contract) throw new Error('Only the game-lab.catalog.v1 MCP or HTTP API contract is supported')
...
async function withMcp<T>(connector: CatalogConnectorManifest, operation: (client: Client) => Promise<T>) {
  ...
  const client = new Client({ name: 'game-lab-catalog', version: app.getVersion() })
  try {
...
export async function testCatalogConnector(idValue: unknown) {
  ...
  if (connector.kind === 'http-api') {
    ...
    return { connected: true, message: `${connector.name} implements game-lab.catalog.v1` }
  }
