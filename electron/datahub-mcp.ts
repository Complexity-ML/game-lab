...
export function normalizeDataHubMcpStartupError(error: unknown, command: string) {
  ...
  if (code === 'ENOENT' || /\bENOENT\b/.test(message)) {
    return new Error(`Local DataHub MCP could not start because "${command}" was not found. Install uv, restart GAME LAB, then connect again.`)
  }
...
async function connectClient(): Promise<Client> {
  ...
  connectionPromise = (async () => {
    const client = new Client({ name: 'game-lab', version: '0.1.0' })
...
export function parseDataHubDecisionRequest(payload: unknown): DataHubDecisionRequest {
  ...
  const author = typeof value.author === 'string' ? value.author.trim().slice(0, 180) : 'GAME LAB operator'
...
export async function writeDataHubDecision(payload: unknown): Promise<{ written: true; tool: 'createDocument' | 'save_document'; summary: string }> {
  ...
  const content = `## GAME LAB approved decision\n\n**Revision:** ${revisionId}\n\n**Author:** ${author}\n\n## Rationale\n\n${rationale}`
  ...
  const result = assertBoundedMcpPayload(await callToolWithSdkTimeout(client, { name: 'save_document', arguments: { document_type: 'Decision', title: `GAME LAB · ${title}`, content, topics: ['game-lab', 'approved-revision'], related_assets: relatedAssets } }, 20_000, 'save_document'), 'save_document response')
  if (result.isError) throw new Error(summarizeResult(result))
