...
beforeEach(() => {
  directory = mkdtempSync(join(tmpdir(), 'game-lab-datahub-'))
...
describe('DataHub MCP connection settings', () => {
  ...
  it('finds uvx in the macOS user install directory even when the app PATH is minimal', () => {
    const expected = '/Users/game-lab/.local/bin/uvx'
    expect(resolveDataHubMcpCommand({ PATH: '/usr/bin:/bin' }, 'darwin', '/Users/game-lab', (candidate) => candidate === expected)).toBe(expected)
    expect(resolveDataHubMcpCommand({ DATAHUB_MCP_COMMAND: '/custom/bin/datahub-mcp' }, 'darwin', '/Users/game-lab', () => false)).toBe('/custom/bin/datahub-mcp')
  })

  it('uses Windows path rules when resolving uvx for a Windows desktop build', () => {
    const expected = 'C:\\Users\\game-lab\\.local\\bin\\uvx.exe'
    expect(resolveDataHubMcpCommand({ PATH: 'C:\\Windows;C:\\Tools' }, 'win32', 'C:\\Users\\game-lab', (candidate) => candidate === expected)).toBe(expected)
  })
  ...
  it('turns a missing uvx spawn failure into an actionable desktop message', () => {
    ...
    expect(normalizeDataHubMcpStartupError(error, 'uvx').message).toContain('Install uv, restart GAME LAB')
  })
  ...
  it('turns autonomous control text into a bounded DataHub search query without Lucene operators', () => {
    expect(buildDataHubSearchQuery('Execute GAME LAB Control: objective=maintain governed graph | mode=autonomous + retry')).toBe('/q Execute+GAME+LAB+Control+objective+maintain+governed+graph+mode+autonomous+retry')
  ...
  it('persists endpoint metadata and an encrypted token without exposing the credential', async () => {
    ...
    expect(electronState.availabilityChecks).toBe(0)
    expect(electronState.decryptions).toBe(0)
    expect(readFileSync(join(directory, 'game-lab.sqlite')).toString('utf8')).not.toContain('datahub-private-token')
  })
