  ...
  run({ nodes }) {
    return nodes.length === 0
      ? [issue(this.id, {
          ...
          detail: 'Add a Game Server or Evidence Source before running the GAME LAB workflow.',
        })]
      : []
  },
}
...
export const pipelineTerminalsAtom: ValidationAtom = {
  ...
  run({ nodes }) {
    ...
    if (!nodes.some((node) => node.data.kind === 'source' || node.data.kind === 'server')) findings.push(issue(this.id, { id: 'missing-source', severity: 'error', title: 'Server or evidence source is required', detail: 'A runnable GAME LAB workflow must start from an authorized Game Server or Evidence Source card.' }))
...
const cardContracts: Partial<Record<CardKind, CardContract>> = {
  control: ({ nodes, edges }, nodeId) => {
    ...
    if (nodes.filter((candidate) => candidate.data.kind === 'control').length > 1) findings.push(issue('card-contracts', {
      ...
      severity: 'error',
      nodeId,
      title: 'Multiple GAME LAB controllers',
      detail: 'Keep exactly one GAME LAB Control card so the autonomous player has one persistent policy.',
    }))
    if (edges.some((edge) => edge.source === nodeId || edge.target === nodeId)) findings.push(issue('card-contracts', {
      ...
      detail: 'GAME LAB Control configures the player globally and must remain outside the dataset lineage path.',
    }))
  ...
  server: ({ nodes, edges }, nodeId) => {
    const node = nodes.find((candidate) => candidate.id === nodeId)
    if (!node) return []
    const findings: ValidationIssue[] = []
    if (edges.some((edge) => edge.target === nodeId)) findings.push(issue('card-contracts', { id: `server-input-${nodeId}`, severity: 'error', nodeId, title: 'Game Server has an input', detail: 'Game Server cards must begin an authorized operational path.' }))
    if (!node.data.serverTelemetry) findings.push(issue('card-contracts', { id: `server-telemetry-${nodeId}`, severity: 'error', nodeId, title: 'Server telemetry is missing', detail: 'Record platform, endpoint, health, players, latency and resource status before operating on a server.' }))
    if (!/scope=private_server/i.test(node.data.rule ?? '') || !/commands=reviewed/i.test(node.data.rule ?? '')) findings.push(issue('card-contracts', { id: `server-safety-${nodeId}`, severity: 'error', nodeId, title: 'Server authorization policy is incomplete', detail: 'GAME LAB supports only authorized private-server scope and reviewed commands.' }))
    return findings
  },
  agent: ({ nodes }, nodeId) => {
    const node = nodes.find((candidate) => candidate.id === nodeId)
    if (!node) return []
    const findings: ValidationIssue[] = []
    if (!node.data.agentTelemetry || node.data.agentTelemetry.safetyMode !== 'private-server-only') findings.push(issue('card-contracts', { id: `agent-safety-${nodeId}`, severity: 'error', nodeId, title: 'Agent safety boundary is missing', detail: 'A Game Agent must be constrained to an authorized private server.' }))
    if (!/act=allowlist/i.test(node.data.rule ?? '') || !/emergency_stop=required/i.test(node.data.rule ?? '')) findings.push(issue('card-contracts', { id: `agent-action-policy-${nodeId}`, severity: 'error', nodeId, title: 'Agent action policy is incomplete', detail: 'Declare allowlisted actions and a required emergency stop before the agent can run.' }))
    return findings
  },
  split: ({ edges }, nodeId) => {
  ...
  run(context) {
    return context.nodes.flatMap((node) => {
      ...
      const globalSystemCard = node.data.kind === 'control'
        || node.data.kind === 'explorer'
        || (node.data.kind === 'worker' && parseWorkerPolicy(node.data.rule).role === 'exploration')
      if (!globalSystemCard && node.data.kind !== 'source' && node.data.kind !== 'server' && node.data.kind !== 'monitor' && !context.edges.some((edge) => edge.target === node.id && edge.sourceHandle !== 'feedback')) findings.push(issue(this.id, { id: `orphan-input-${node.id}`, severity: 'error', nodeId: node.id, title: 'Orphan card', detail: `${node.data.label} does not receive data.` }))
