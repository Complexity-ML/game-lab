...
export function repairSensitiveOutputPaths(
  ...
  if (repairedOutputs.length) {
    ...
    proposal.summary = `${proposal.summary} GAME LAB inserted ${repairedOutputs.length} mandatory sensitive-data protection boundary${repairedOutputs.length === 1 ? '' : 'ies'} required by atomic validation.`
...
export function repairMonitorWorkBranches(
  ...
  if (repairedMonitors.length) {
    proposal.summary = `${proposal.summary} GAME LAB connected ${repairedMonitors.length} Live Monitor${repairedMonitors.length === 1 ? '' : 's'} to the first replayable evidence card of the bounded iteration.`
