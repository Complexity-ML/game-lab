...
export function buildCardActivationPlan(
  ...
  const sources = nodes.filter((node) => node.data.kind === 'source' || node.data.kind === 'server')
    ...
    switch (kind) {
      ...
      case 'query':
        return sources.length > 0 && (profiles.length === 0 || aggregateCoverageGap)
          ...
          : undefined
      case 'server':
        return !presentKinds.has('server') && nodes.length === 0
          ? 'A Server Ops or Agent Arena workflow needs one authorized private server root.'
          : undefined
      case 'agent':
        return presentKinds.has('server') && !presentKinds.has('agent') && /\bagent|npc|mission|replay|test player\b/i.test(findingsText)
          ? 'The operator objective requires one governed NPC or test-player card.'
          : undefined
      case 'source':
