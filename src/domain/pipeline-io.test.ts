...
import { customerActivationEdges, customerActivationNodes, loadPipelinePreset, newCard } from './pipeline'
...
describe('versioned pipeline JSON exchange', () => {
  ...
  it('rejects unsupported schema versions before returning a graph', () => {
    expect(() => parsePipelineExport(JSON.stringify({ schema: 'game-lab.pipeline', schemaVersion: 99, graph: { nodes: [], edges: [] } }))).toThrow('Unsupported GAME LAB schema version 99')
  })
  ...
  it('round-trips bounded Game Server and Game Agent telemetry', () => {
    const preset = loadPipelinePreset('agent-arena')
    const imported = parsePipelineExport(JSON.stringify(createPipelineExport(preset.title, preset.nodes, preset.edges, [])))
    expect(imported.graph.nodes.find((node) => node.data.kind === 'server')?.data.serverTelemetry).toMatchObject({
      platform: 'FiveM',
      state: 'online',
      playersOnline: 1,
      playerCapacity: 8,
    })
    expect(imported.graph.nodes.find((node) => node.data.kind === 'agent')?.data.agentTelemetry).toMatchObject({
      mode: 'test-player',
      state: 'acting',
