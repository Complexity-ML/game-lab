...
describe('optional judge-readable presets', () => {
  it.each(['server-ops', 'agent-arena'] as const)('loads the %s GAME LAB workflow as an authorized private-server loop', (id) => {
    const preset = loadPipelinePreset(id)
    const server = preset.nodes.find((node) => node.data.kind === 'server')
    const monitor = preset.nodes.find((node) => node.data.kind === 'monitor')
    const output = preset.nodes.find((node) => node.data.kind === 'output')
    expect(server?.data.serverTelemetry?.platform).toBe('FiveM')
    expect(server?.data.rule).toContain('scope=private_server')
    expect(preset.nodes.some((node) => node.data.kind === 'review')).toBe(true)
    expect(preset.nodes.some((node) => node.data.kind === 'validation')).toBe(true)
    expect(preset.edges).toContainEqual(expect.objectContaining({ source: output?.id, target: monitor?.id, sourceHandle: 'feedback' }))
    expect(validatePipeline(preset.nodes, preset.edges).filter((finding) => finding.severity === 'error')).toEqual([])
  })

  it('keeps the Agent Arena action surface allowlisted and stoppable', () => {
    const preset = loadPipelinePreset('agent-arena')
    const agent = preset.nodes.find((node) => node.data.kind === 'agent')
    expect(agent?.data.agentTelemetry).toMatchObject({ mode: 'test-player', safetyMode: 'private-server-only' })
    expect(agent?.data.rule).toContain('act=allowlist')
    expect(agent?.data.rule).toContain('emergency_stop=required')
  })
