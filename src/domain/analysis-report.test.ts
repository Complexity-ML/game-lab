...
describe('analysis report', () => {
  it('summarizes server telemetry and agent safety without software-license semantics', () => {
    const preset = loadPipelinePreset('agent-arena')
    const report = buildAnalysisReport(preset.nodes, collectRiskImpactOverview(preset.nodes, preset.edges))
    expect(report.mode).toBe('game')
    expect(report.scope).toBe('Driver 01 private agent evaluation')
    expect(report.summary).toContain('Agent Arena is online')
    expect(report.summary).toContain('private-server-only')
    ...
    expect(report.limitations.join(' ')).toContain('public-server automation')
  })
