...
const gameIntent = /\b(agent|analyse|analyze|arena|audit|blocks?|bridge|cards?|cartes?|diagnose|diagrams?|fivem|game|graphs?|graphes?|incidents?|inventory|latency|minecraft|mission|monitors?|npc|players?|replay|resources?|risks?|risques?|server|telemetry|validation|world|workspaces?)\b/i
...
export function gameDiscoveryQuery(objective: string): string {
...
export function resolveAgentObjective(rawObjective: string, options: { hasGraph: boolean; matchedSource: boolean }): AgentObjectiveResolution {
  ...
  const accepted = gameIntent.test(objective)
