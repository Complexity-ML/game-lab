...
export function buildPipelineAgentRequest(input: AgentContextInput & {
  ...
  return {
    ...
    guardrails: [
      ...
      "For Minecraft, use only the structured inventory, nearbyBlocks, localMap and entity IDs in gameRuntime; order motor steps by dependency, choose routes that avoid localMap hazard, blocked and drop cells, and never invent a world block, recipe or coordinate. A craft_item target may use an exact standard Minecraft registry item identifier, which the local Crafting Motor validates before execution. If a later step depends on unknown post-action state, end the plan there so the host can ask GPT once from fresh evidence",
      "For Minecraft craft_item, request the final desired item and count only. The local Crafting Motor resolves the full recipe graph, reports available and missing ingredients, crafts intermediates, creates and safely places a crafting table when required, and validates every local micro-action. Do not spend GPT motor steps on predictable planks, sticks or crafting-table intermediates",
      "Never queue Minecraft craft_item when the fresh structured inventory cannot satisfy its reported base-material dependencies. Mine and collect compatible nearby resources first, end the motor plan when collection changes inventory, then replan crafting from the validated inventory",
      "For Minecraft mine_block, the local adapter selects and equips the fastest suitable available harvesting tool. If the required tool is absent it fails without destroying the target, so plan collection and one final craft_item tool objective first",
