...
const kinds: CardKind[] = ['control', 'explorer', 'worker', 'query', 'server', 'agent', 'source', 'profile', 'analysis', 'impact', 'risk', 'patch', 'monitor', 'parallel', 'diagram', 'split', 'decision', 'transform', 'review', 'validation', 'output']
...
describe('card compatibility', () => {
  ...
  it('starts server operations at a private server and routes monitored work into an agent', () => {
    expect(canConnectCardKinds('server', 'monitor')).toBe(true)
    expect(canConnectCardKinds('server', 'agent')).toBe(true)
    expect(canConnectCardKinds('monitor', 'agent')).toBe(true)
    expect(canConnectCardKinds('agent', 'analysis')).toBe(true)
    expect(canConnectCardKinds('analysis', 'server')).toBe(false)
  })
