...
function cleanNodeData(data: Record<string, unknown>, trustHostProof: boolean): PipelineNodeData {
  ...
  const boundedNumber = (value: unknown, minimum: number, maximum: number) => typeof value === 'number' && Number.isFinite(value)
  ...
  return {
    ...
    serverTelemetry: kind === 'server' && rawServer ? {
      platform: ['FiveM', 'RedM', 'Generic'].includes(String(rawServer.platform)) ? rawServer.platform as 'FiveM' | 'RedM' | 'Generic' : 'Generic',
