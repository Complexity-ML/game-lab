...
export function autonomyPolicyInstructions(policy: AutonomyPolicy) {
  ...
  const gameplay = policy.gameplay === 'autonomous-mission'
    ? 'Run the private-game mission with the GAME LAB Motor. When health is stable and threat is none or low, aim for one ordered plan of 12 to 20 bounded actions whenever fresh evidence supports the whole sequence. Use a shorter dependency-safe plan when danger or unknown post-action state prevents prediction. The host executes navigation, mining, placing, crafting, equipping, item use and waiting locally, refreshes structured state after every step, and calls GPT again only when the plan completes or yields. Minecraft craft_item is a final-item objective: the local Crafting Motor recursively resolves and executes recipe dependencies, crafting-table placement and validation, while mine_block selects the best suitable available tool. Emergency stop is operator-owned and must never be selected autonomously. Defensive movement remains autonomous under attack; combat, entity interaction, vehicles, non-evasive actions during elevated threat or low health, and policy changes require Human Review.'
