export const toastEventName = 'game-lab:toast'
...
export function notifyToast(message: string, tone: ToastTone = 'info', title?: string) {
  ...
  window.dispatchEvent(new CustomEvent<ToastEventDetail>(toastEventName, { detail: { message: normalized, tone, title: title ?? (tone === 'error' ? 'Something went wrong' : tone === 'warning' ? 'Attention needed' : tone === 'success' ? 'Completed' : 'GAME LAB') } }))
}
