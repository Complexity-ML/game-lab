import { AlertTriangle, CheckCircle2, FileCheck2, Gamepad2, PanelRightClose, Server, ShieldAlert, Sparkles } from 'lucide-react'
  ...
  return <>
    ...
    <PanelScrollArea className="reports-panel-content" label="Analysis results content">
      ...
      {report.evidence.length ? <div className="analysis-result-evidence">{report.evidence.map((item) => <button aria-label={`Inspect evidence ${item.title}`} key={item.nodeId} onClick={() => onSelectCard(item.nodeId)} type="button">
        {item.kind === 'server' ? <Server size={15} /> : item.kind === 'agent' ? <Gamepad2 size={15} /> : <FileCheck2 size={15} />}
        <span><small>{item.label}</small><strong>{item.title}</strong><p>{item.detail}</p></span>
      </button>)}</div> : <div className="reports-clear"><FileCheck2 size={18} /><span><strong>No GAME LAB output yet</strong><small>Add server telemetry, an incident analysis or an agent replay result.</small></span></div>}
      ...
      <section className={`analysis-limitations${report.limitations.length ? ' has-gaps' : ''}`}>
        ...
        {report.limitations.length
          ...
          : <p>No game telemetry coverage gap is recorded in the current graph.</p>}
      </section>
