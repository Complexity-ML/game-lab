import { AlertTriangle, BellRing, Check, CheckCircle2, Clock3, Gamepad2, GitBranch, RotateCcw, Square, X, XCircle } from 'lucide-react'
...
export function VersionBrowser({ onApprove, onEmergencyStop, onReject, onRemind, onRestore, pipelineTitle, selectedVersionId, versions }: VersionBrowserProps) {
  ...
  return <div className="version-browser">
    ...
    <article className="version-browser-detail">
      ...
      <section className="version-evidence"><h5><Gamepad2 size={13} />Game evidence provenance</h5>{selected.evidence?.length ? <ol>{selected.evidence.map((item, index) => <li className={item.stale || item.status !== 'ok' ? 'is-stale' : ''} key={`${item.tool}-${item.source}-${index}`}><span><strong>{item.tool}</strong><em>{item.cached ? 'cache' : 'live'} · {item.status}</em></span><code>{item.source}</code><p>{item.summary}</p><small>Captured {new Date(item.capturedAt).toLocaleString()} · {item.stale ? 'stale/unavailable' : `valid until ${new Date(item.expiresAt).toLocaleTimeString()}`}</small></li>)}</ol> : <p>No game evidence was attached to this revision.</p>}</section>
