...
export function useDiagnosticsActions(setIncidentEvents: Dispatch<SetStateAction<IncidentEvent[]>>) {
  const exportDiagnostics = async () => {
    if (!window.gameLab) { notifyError('Diagnostics require the Electron application'); return }
    try {
      const bundle = await window.gameLab.exportDiagnostics()
      const url = URL.createObjectURL(new Blob([JSON.stringify(bundle, null, 2)], { type: 'application/json' }))
      const link = document.createElement('a')
      link.href = url
      link.download = `game-lab-diagnostics-${new Date().toISOString().slice(0, 10)}.json`
      link.click()
  ...
  const openLogs = async () => {
    if (!window.gameLab) { notifyError('Diagnostics require the Electron application'); return }
    try { await window.gameLab.openDiagnosticLogs() } catch (error) { notifyError(error, 'Unable to open diagnostic logs') }
  }

  const loadBundle = async () => {
    if (!window.gameLab) throw new Error('Diagnostics require the Electron application')
    return window.gameLab.exportDiagnostics()
  }

  const clearIncidentReports = async () => {
    if (!window.gameLab?.clearIncidentEvents) throw new Error('Incident cleanup requires the Electron application')
    const result = await window.gameLab.clearIncidentEvents()
    setIncidentEvents([])
