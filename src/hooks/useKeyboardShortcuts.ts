...
export function useKeyboardShortcuts(actions: KeyboardShortcutActions) {
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (isEditing) return
      if (event.altKey && event.key === '1') { event.preventDefault(); focusRegion('game-lab-library'); return }
      if (event.altKey && event.key === '2') { event.preventDefault(); focusRegion('game-lab-canvas'); return }
      if (event.altKey && event.key === '3') { event.preventDefault(); focusRegion('game-lab-inspector'); return }
      if (event.altKey && event.key === '4') { event.preventDefault(); focusRegion('game-lab-agent-prompt'); return }
