...
export function useDataHubConnection(setActivity: (message: string) => void) {
  ...
  const saveCatalogConnector = async (payload: CatalogConnectorManifest & { token?: string; clearToken?: boolean }) => {
    ...
    if (!window.dataLab.saveCatalogConnector) throw new Error('This GAME LAB build does not support custom catalog connections')
  ...
  const deleteCatalogConnector = async (id: string) => {
    if (!window.dataLab) throw new Error('Catalog connections require the Electron application')
    if (!window.dataLab.deleteCatalogConnector) throw new Error('This GAME LAB build does not support custom catalog connections')
  ...
  const testCatalogConnector = async (id: string) => {
    if (!window.dataLab) throw new Error('Catalog connections require the Electron application')
    if (!window.dataLab.testCatalogConnector) throw new Error('This GAME LAB build does not support custom catalog connections')
