...
export function usePipelineInteractions(options: {
  ...
  const dropLibraryCard = (event: DragEvent<HTMLDivElement>) => {
    ...
    const rawKind = event.dataTransfer.getData('application/game-lab-card')
  ...
  const exportPipelineJson = () => {
    ...
    link.href = url
    link.download = `${options.projectTitle.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'game-lab-pipeline'}.json`
    link.click()
  ...
  const fitCommittedGraph = (nodeIds?: Iterable<string>) => {
    const focusNodes = [...new Set(nodeIds ?? [])]
    ...
    window.requestAnimationFrame(() => {
      ...
      window.requestAnimationFrame(() => {
        void flowInstance.current?.fitView({
          duration: 240,
          padding: focusNodes.length > 0 ? 0.28 : 0.22,
          ...(focusNodes.length > 0 ? { nodes: focusNodes } : {}),
        })
      })
    })
  }
