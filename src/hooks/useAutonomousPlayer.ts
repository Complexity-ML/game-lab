...
export function useAutonomousPlayer(options: AutonomousPlayerOptions) {
  ...
  const auditWithAgent = async (agentRequest = defaultBlankObjective, monitored?: LiveIncidentTrigger, expectedPlayerSessionId?: number) => {
    ...
    if (!objective.accepted) {
      setActivity('Request outside GAME LAB scope · no provider call · graph unchanged')
    ...
    const gameServers = nodes.filter((node) => node.data.kind === 'server')
    const hasDataSource = nodes.some((node) => node.data.kind === 'source' || node.data.kind === 'server')
    ...
    try {
      if (routedSources.length > 0) {
        ...
        datahubEvidence = gameServers.length
          ? gameServers.map((server) => {
              const telemetry = server.data.serverTelemetry
              return telemetry
                ? `Authorized private game server ${server.data.label}: platform=${telemetry.platform}; state=${telemetry.state}; players=${telemetry.playersOnline}/${telemetry.playerCapacity}; latency_ms=${telemetry.latencyMs}; cpu_percent=${telemetry.cpuPercent}; memory_mb=${telemetry.memoryMb}; resources_running=${telemetry.resourcesRunning}; resources_failed=${telemetry.resourcesFailed}. Treat this versioned snapshot as bounded telemetry and never invent player-level data.`
                : `Authorized private game server ${server.data.label} has no bounded telemetry snapshot. Record a collection gap and do not invent operational state.`
            })
          : ['No bounded private-game evidence matched the prompt. Treat evidence as incomplete and do not modify an unrelated branch.']
      }
      ...
      const requestPayload = buildPipelineAgentRequest({
        ...
        sourceScope: {
          ...
          sourceIds: [...routedSources.map((source) => source.id), ...gameServers.map((server) => server.id)],
      ...
      if (incidentSummaries.some((incident) => incident.incidentKey === providerConnectivityKey && incident.status !== 'resolved')) {
        await logIncident({
          ...
          sourceSystem: 'GAME LAB connectivity',
      ...
      if ((monitored || autonomousSessionActive) && !nextProposal.requiresHumanReview && !touchesReviewCheckpoint) {
        ...
        if (autonomousVersionId) {
          ...
          if (repair.shouldRetry) {
            ...
            notifyToast('The graph stayed unchanged. GAME LAB exhausted the bounded repair turn and will not hot-loop on the same invalid diff.', 'error', 'Atomic repair stopped')
            recordDiagnostic({
              category: 'revision',
  ...
  const playAgent = () => {
    ...
    if (systemCards.added.length) {
      ...
      queueAutonomousStep(
        `Execute the persistent GAME LAB Control policy as coherent versioned iterations: ${controller.data.rule}`,
        sessionId,
        0,
      )
      return
    ...
    const objective = controller?.data.rule?.trim()
      ? `Execute the persistent GAME LAB Control policy exactly and incrementally: ${controller.data.rule}`
    ...
    try {
      ...
      if (approvalBlockers.length) {
        ...
        if (repair.shouldRetry) {
          ...
          notifyToast('The invalid diff was discarded. GAME LAB will wait for a new event or a new Human Review instead of retrying forever.', 'error', 'Atomic repair stopped')
        }
        return true
      }
