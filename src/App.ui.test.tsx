...
function installElectronWorkspaceMock(state: Awaited<ReturnType<NonNullable<typeof window.dataLab>['loadWorkspaceState']>>) {
  ...
  const api = {
    ...
    openDiagnosticLogs: vi.fn(async () => ({ opened: true as const, path: '/tmp/game-lab-diagnostics.json' })),
...
describe('visual pipeline workspace regressions', () => {
  it('opens a new install blank, with zero versions and no false successful run state', async () => {
    ...
    expect(screen.queryByRole('textbox', { name: 'What should the GAME LAB agent do?' })).toBeNull()
  })
  ...
  it('uses one bounded license discovery for an empty autonomous workbench', async () => {
    ...
    expect(api.searchDataHubAssets).toHaveBeenCalledWith('game server')
  ...
  it('keeps Reports and Results as separate entries alongside the other workspace panels', async () => {
    ...
    expect(closeActions.closest('#game-lab-left-panel')).toBeTruthy()
    ...
    expect(screen.getByRole('button', { name: 'Close live logs' }).closest('#game-lab-left-panel')).toBeTruthy()
    ...
    expect(screen.getByText('No material game finding')).toBeTruthy()
  ...
  it('returns from a risk card inspector to the preserved Risks panel', async () => {
    ...
    await user.click(screen.getByTitle('Click to add or drag Operational Risk onto the canvas'))
    await user.click(screen.getByRole('button', { name: 'Open impact and risks' }))
    ...
    await user.click(screen.getByRole('button', { name: 'Inspect New Operational Risk' }))
  ...
  it('persists autonomy controls and exposes review, risk and uncertainty policies', async () => {
    ...
    expect(JSON.parse(window.localStorage.getItem('game-lab-autonomy-policy') ?? '{}')).toEqual({
  ...
  it('adds palette cards by click and drops dragged cards at pointer flow-space XY', async () => {
    ...
    const flow = screen.getByTestId('pipeline-flow')
    const sourcePaletteCard = screen.getByTitle('Click to add or drag Evidence Source onto the canvas')
    ...
    const analysisPaletteCard = screen.getByTitle('Click to add or drag Game Analysis onto the canvas')
  ...
  it('creates a durable SQLite workspace when the blank workbench receives its first card', async () => {
    ...
    await user.click(screen.getByTitle('Click to add or drag Evidence Source onto the canvas'))
  ...
  it('does not autosave an example over the last active workspace', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'FiveM Server OpsDiagnose one failed resource, review the recovery and validate the private server.' }))

    expect(await screen.findByText('FiveM server incident response')).toBeTruthy()
  ...
  it('commits an approved provider proposal into the visible main graph', async () => {
    ...
    api.runChatGPTProposal = vi.fn(async () => ({
      ...
      proposal: {
        ...
        actions: [
          { type: 'add_card', node_id: 'source-intended-dataset', kind: 'source', label: 'Intended Dataset', description: 'Awaiting a DataHub binding.', owner: 'GAME LAB Agent', rule: 'Bind a trusted dataset before execution.', source: null, target: null, source_handle: null, reason: 'Create the bounded source placeholder.' },
...
describe('agent prompt regressions', () => {
  it('does not open Settings when Enter is pressed while disconnected', () => {
    ...
    const textarea = screen.getByRole('textbox', { name: 'What should the GAME LAB agent do?' })
  ...
  it('caps multi-line growth, keeps scrolling on the textarea and leaves actions outside it', () => {
    ...
    expect(within(form!).getByRole('button', { name: 'Send request to GAME LAB agent' })).toBeTruthy()
